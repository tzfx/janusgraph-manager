...
describe('GraphIndexBuilder', () => {
    it('should be instantiated with a name and key', () => {
        ...
        expect(out).toContain(`mgmt.buildIndex('test', Vertex.class).addKey(mgmt.getPropertyKey('testfield'), Mapping.STRING.getParameter()).buildCompositeIndex();`);
    });
    ...
    it('should be instantiated with a name and multiple keys', () => {
        ...
        expect(out).toContain(`.addKey(mgmt.getPropertyKey('testfield1'), Mapping.STRING.getParameter())`);
        expect(out).toContain(`.addKey(mgmt.getPropertyKey('testfield2'), Mapping.STRING.getParameter())`);
    });
    ...
    it('should ignore duplicate keys', () => {
            ...
            const out = gib.key(key).key({...key}).build();
            expect(out).toContain(`mgmt.buildIndex('test', Vertex.class).addKey(mgmt.getPropertyKey('testfield'), Mapping.STRING.getParameter()).buildCompositeIndex();`);
    })
